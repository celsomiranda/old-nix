...
{ config, lib, pkgs, ... }: {
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (config.celso.machine.flavor != "wsl") {
      # Boot
      ...
      # Network
      networking.hostName = config.celso.machine.name;
    ...
    (lib.mkIf (config.celso.machine.flavor == "wsl") {
      wsl = {
        enable = true;
        automountPath = "/mnt";
        defaultUser = config.celso.machine.user.name;
      ...
      system.activationScripts.copy-home-launchers = lib.stringAfter [ ] ''
        ...
        for x in applications icons; do
          ...
          ${pkgs.rsync}/bin/rsync -ar /etc/profiles/per-user/${config.celso.machine.user.name}/share/$x/. ./$x
          mkdir -p /usr/share/$x
    ...
    (lib.mkIf (config.celso.machine.role != "server") {
      # For GTK stuff
    ...
    # Laptop configs
    (lib.mkIf (config.celso.machine.role == "laptop") {
      # Backlight
